...
if 'model_preference' not in st.session_state:
    st.session_state['model_preference'] = "GLM-4.7 (Free/Fast)"

uploaded_file = None

# --- 3. SIDEBAR ---
# --- MODEL SELECTOR ---
model_choice = st.sidebar.radio("Select Engine:", ("GLM-4.7 (Free/Fast)", "Claude 3.5 (Paid/Smart/Vision)"), key='model_radio', index=0 if st.session_state['model_preference'] == "GLM-4.7 (Free/Fast)" else 1)

# Update preference if manually changed
if model_choice != st.session_state['model_preference']:
    st.session_state['model_preference'] = model_choice
...
if st.session_state['tab'] == 'Chat':
    ...
    if prompt := st.chat_input("Ask a question..."):
        ...
        if prompt.strip().lower() == "/claude":
            st.session_state['model_preference'] = "Claude 3.5 (Paid/Smart/Vision)"
            st.info("🔄 Switched to Claude 3.5 (Paid/Smart/Vision)")
            st.rerun()
        elif prompt.strip().lower() == "/glm":
            st.session_state['model_preference'] = "GLM-4.7 (Free/Fast)"
            st.info("🔄 Switched to GLM-4.7 (Free/Fast)")
            st.rerun()
